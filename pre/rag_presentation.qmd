...
- Reranking is introduced in the retrieval process, and sorted the preliminary search results by the `CrossEncoder` model (cross-encoder/ms-marco-MiniLM-L-12-v2).
- In the reordering stage, semantic matching scores of query and candidate document pairs are used to further optimize the search results
...
[Link to Contextual Retrieval Augmented Generation (RAG) System](https://yuting-fan-265-rag-system-rag-app-new-v1sxnc.streamlit.app)
:::

:::

::: {.column width="50%"}
- Real-time Document Upload: Users can update the knowledge base on the fly.
- Multiple GPT Models: Choose between GPT-3.5 and GPT-4 for generation.
- Retrieval Transparency: Source of retrieved data is embedded in responses.
:::
...
- Document Uploaded: Declaration of Independence
- Query: What is the attitude of this writer?

![](images/example.png){fig-align="center" style="width: 100%"}
